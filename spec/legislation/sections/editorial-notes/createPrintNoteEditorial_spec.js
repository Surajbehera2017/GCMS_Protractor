...
describe('Editorial Notes', function () {
	...
	it('Create an print note and save it.'+jiraNumber, function () {
		...
		editorialNotes.clickOnElement(loadedData.createprintnote);
		browser.sleep(8000);
		browser.getAllWindowHandles().then(function(handles) {
			var newTabHandle = handles[1];
			browser.switchTo().window(newTabHandle).then(function () {
		...
		browser.waitForAngular();
		browser.sleep(2000);
		 expect(global.gettingTextofelement(loadedData.verifyprinttext,printNoteText)).toEqual(true);
