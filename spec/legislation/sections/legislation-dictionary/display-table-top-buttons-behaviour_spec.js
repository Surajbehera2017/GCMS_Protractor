...
describe('Legislation Dictionary', function () {
    ...
    beforeEach(function () {
   	...
   	it('should click on the view all link once the section is expanded and also check import,add,show filter and edit button is present or not',function(){

        legislationdictionary.clickViewAllLink();
        browser.waitForAngular();
        expect(legislationdictionary.displayViewAllLink()).toEqual(true);
		expect(legislationdictionary.isPresentEditButton()).toEqual(true);
		expect(legislationdictionary.isPresentImportButton()).toEqual(true);
		expect(legislationdictionary.isPresentAddButton()).toEqual(true);
		expect(legislationdictionary.isPresentShowFilterButton()).toEqual(true);
		legislationdictionary.clickAdd();
		expect(legislationdictionary.isDisplayAdd()).toEqual(true);
		legislationdictionary.clickCrossButtonOnAddPage();
		legislationdictionary.clickShowFilterButton();
		expect(legislationdictionary.isPresentShowFilterButton()).toEqual(true);
		legislationdictionary.clickcloseDicTable();
	})
});