...
describe('Corrigendas', function () {
	...
	it('check showandHide filter,cancel button are working properly',function(){
		browser.waitForAngular();
		corrigendas.clickViewAllLink();
		browser.waitForAngular();
		corrigendas.clickShowAndHideFilter();
		expect(corrigendas.isVisibleHideFilter()).toEqual(true);
		corrigendas.clickCancelCorrigendasButton();
		expect(corrigendas.isVisibleHideFilter()).toEqual(false);
	});
