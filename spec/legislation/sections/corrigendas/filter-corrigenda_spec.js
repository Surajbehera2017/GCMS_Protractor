...
describe('Corrigendas', function () {
	...
	it('should get the data as per given filters for corrigenda ',function(){
		browser.waitForAngular();
		corrigendas.clickViewAllLink();
		browser.waitForAngular();
		corrigendas.clickShowAndHideFilter();
		corrigendas.enterMarginalNumber(loadedData.marginal_id);
		corrigendas.getCountOfRowsDisplayedInTable().then(function(count){
			if(count===0){
				expect(count).toEqual(0);
			}else{
				expect(corrigendas.getFirstMarginalNumber()).toEqual(loadedData.marginal_id);
			}
			corrigendas.enterMarginalNumber('');//clear filter
		});
	});
