...
var jiraNumber = 'GCMSQABANG-1624';
...
describe('Document-structure', function () {
	...
	afterEach(function () {
		...
		browser.sleep(4000);
		rightpanelpage.rightClickOnVersionAndSelectDeleteLayer(loadedData.sibling_unit_overline,'Original');
		...
		rightpanelpage.clickDocumentStructure();
		browser.sleep(4000);
		rightpanelpage.rightClickOnUnitAndSelectDeleteDropdown(loadedData.sibling_unit_overline);
		rightpanelpage.clickYesButtonInDeleteUnitPopUp();
		rightpanelpage.clickOnButtonForGlobal();
	...
	/*
	 * doubt TC13 -  Right Panel - Document Structure - Unit Menu - Add a Unit from Structure using Xeditor - Overline
	 */
	
	 it('Add Unit Xeditor Unit Menu Added as Lower Sibling Overline.'+jiraNumber, function () {
		...
		*/
		...
		rightpanelpage.writeUnitIDInAddUnitPopUp(loadedData.sibling_unit_overline);
		...
		rightpanelpage.changeTheWordFormat('overline');
		...
		infoMsg.then(function(text) {
			if(text===''){ //Save successful. No info message. Popup closes automatically
				...
				rightpanelpage.rightClickOnVersionAndSelectEditLayerXML(loadedData.sibling_unit_overline,'Original');
				browser.sleep(5000);
				browser.waitForAngular();
				//Verify Overline text tag for paragraph_two
				var newXMLText = rightpanelpage.getXMLTextInUnitPopUp();
				expect(newXMLText).toContain("<en-origen decoracion-texto=\"superrayado\">"+paragraph_one+"</en-origen>");
			}
			else{//info message displayed. Duplicate entry for unit, hence cannot save. expected.
				console.log('Overline is not verified. Given unit already exist in structure. Trying to delete it');
