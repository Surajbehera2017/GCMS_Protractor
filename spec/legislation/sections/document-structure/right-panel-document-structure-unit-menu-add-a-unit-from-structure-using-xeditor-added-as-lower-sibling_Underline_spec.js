...
var jiraNumber='GCMSQABANG-1623';
...
describe('Document-structure', function () {
	...
	afterEach(function () {
		...
		browser.sleep(4000);
		rightpanelpage.rightClickOnVersionAndSelectDeleteLayer(loadedData.sibling_unit_underline,'Original');
		...
		rightpanelpage.clickDocumentStructure();
		browser.sleep(4000);
		rightpanelpage.rightClickOnUnitAndSelectDeleteDropdown(loadedData.sibling_unit_underline);
		rightpanelpage.clickOnApplyOrCloseButton('ok');
		//click ok button
		rightpanelpage.clickOnApplyOrCloseButton('ok');
	...
	/*
	 *GCMSQABANG-1623: TC12 -  Right Panel - Document Structure - Unit Menu - Add a Unit from Structure using Xeditor - Underline
	 */
	it('Add Unit Xeditor Unit Menu Added as Lower Sibling Underline.'+jiraNumber, function () {
		//Set up data
		...
		*/
		...
		rightpanelpage.writeUnitIDInAddUnitPopUp(loadedData.sibling_unit_underline);
		...
		rightpanelpage.changeTheWordFormat('underline');
		...
		infoMsg.then(function(text) {
			if(text===''){ //Save successful. No info message. Popup closes automatically
				...
				rightpanelpage.rightClickOnOrignalAndXmlLayer(loadedData.sibling_unit_underline,'Original');
				browser.sleep(5000);
				browser.waitForAngular();
				//Verify Underline text tag for paragraph_two
				var newXMLText = rightpanelpage.getXMLTextInUnitPopUp();
				//expect(newXMLText).toContain("<en-origen decoracion-texto=\"subrayado\">"+paragraph_two+"</en-origen>");
				...
				console.log('Underline is not verified. Given unit already exist in structure. Trying to delete it');
