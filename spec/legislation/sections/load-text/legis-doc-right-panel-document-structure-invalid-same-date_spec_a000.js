...
describe('Legislation Document Display - Right Panel - Document Structure', function () {
	...
	beforeEach(function () {
       ...
       /*browser.ignoreSynchronization = true;
        ...
        legisDocDisplayPage.get(testData.legislation.sections.load_text.marginal_id_duplicate_structure);
var loginpopup =  element(by.model('credentials.username'));
	...
	it('should verify invalid same date error message', function () {
                ...
                section.clickFirstUnitRow.then(function(){
                    section.clickOnStructureActions.then(function(){
                        section.clickOnDuplicateStructure.then(function(){
                            section.enterNextStructureDateField('08/08/2016').then(function(){
                               section.clickOnOkBtnOnDuplicateStrucPopup.then(function(){
                                   expect(section.getErrorMessageOnDuplicateStructure).toEqual('A structure already exists with the same date.');
                               }); 
                            });
                        });
                    });
