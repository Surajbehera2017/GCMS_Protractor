     ...
     for (var i=0; i < totalSuites; i=count) {
        ...
        //get test cases results
		  ...
		  if (testCasesResults2[j] != 'Failed' && report.screenshotsOnlyOnFailure) {
           ...
           if (report.modifiedSuiteName) {
             screenshotsNamesOnFailure2.push(report.browser +'-'+ suite.name.substring(suite.name.indexOf(".")+1) + ' ' + testCasesNames2[j] + '.png');
            }
           else {
              screenshotsNamesOnFailure2.push(report.browser +'-'+ suite.name + ' ' + testCasesNames2[j] + '.png');
            }
          }
