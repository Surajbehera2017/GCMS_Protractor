...
LegislationBillsSection.prototype = Object.create({}, {
    ...
    _scrollTo: {
		value: function (webelement) {
			// There are some issues with the double scroll when trying to click
			// some elements
    ...
    clickViewAllLink= function() {
       element(by.id('contextIndexViewAllLink')).click(); 
    };
    ...
    _buttonPencil:{
    	get: function(){
    		 // return
				// element(by.xpath('//div[@id="billsTableContainer"]/div[1]/div[7]/div[1]/div[1]/div[12]//div[@class="ng-scope"]/div[1]/span[1]'));
    ...
    _popupProcedure:{
    	get: function(){
    		// return
			// element(by.xpath('.//*[@id="formNuevaFaseIniciativa"]/table[2]/tbody/tr[3]/td[1]/input[3]'));
    ...
    selectPublication:{
        value: function (str){
            ...
            // return
			// browser.driver.findElement(by.css('[alt="buscar"][class="buttonInform"]')).click().then(function(){
 ...
 selectLegislativeBodySearch:{
   ...
   element(by.css('#legislativeBodyStatueType0 a[name="lnkSearchDropdown"]')).click();
    ...
    _legisstatueTypedropbox= function () { return element(by.id('legislativeBodyStatueType0')).element(by.css('input[ng-keypress="searchOnEnter($event)"]'))},
    
    clickprovisonDateButton=function(){
            element(by.id('add.provision.date-0-')).click();
      },
    
    _provisonDateButton1:{get: function () {return element(by.id('add.provision.date-0-'))}},
    
    clickconnectorComboBox=function(){
           element.all(by.css('div[id^="connetor-col-1"]')).get(0).click();
    },
    
    _connectorComboBox:{get : function () {return element.all(by.css('div[id^="connetor-col-1"]')).get(0)}},
    
    selectconnector= function () {
             element(by.css('div[id^="connetor-col-1"]'));
    },
    
    clickMainKeyWordcolobule= function () {
           
            element(by.css('.form-control.ng-valid.has-visited.ng-dirty')).sendKeys("Poder Judicial");
    },
    
     _MainKeyWordcolobule:{get:function (){return element(by.css('.form-control.ng-valid.has-visited.ng-dirty'));}},
    
    clickMainKeyWord=function(){
    element(by.css('.form-control.ng-valid.has-visited.ng-dirty')).sendKeys(protractor.Key.ENTER);
 
},
   
    legismainKeyword= function (value) {
            element(by.css('.form-control.ng-valid.has-visited.ng-dirty')).clear().sendKeys(value);
       },
    
    _mainKeyword=function () 
    { 
    	element(by.css('div[id="keyword-container"]')).element(by.css('input[name="txtdropdownsearch"]'));
    	   
       },
    
    clickCreateButton=function(){
     element(by.id('btnSaveDocument')).click();
    },
    
  clickBillsAddButton=function(){
     element(by.css('[ng-click="sectionBillsController.goToAdd()"]')).click();
  },
    
    clickNewPublicationButton= function(){
